...
void RELAPACK(csytrf)(const char *uplo, const int *n,
    ...
    *info = 0;
    ...
    float *W = Work;
    if (*lWork < *ldA * *n)
        W = malloc(*ldA * *n * 2 * sizeof(float));

    int nout;
    RELAPACK(csytrf_rec)(uplo, n, n, &nout, A, ldA, ipiv, W, ldA, info);

    if (W != Work)
        free(W);
}

void RELAPACK(csytrf_rec)(const char *uplo,
    ...
    if (*n <= MAX(CROSSOVER_CSYTRF, 3)) {
        ...
        if (*n == *n_full) {
            ...
            *n_out = *n;
            ...
            LAPACK(csytrf_rec2)(uplo, n_full, n, n_out, A, ldA, ipiv, Work, ldWork, info);
        return;
    }
    ...
    if (lower) {
        ...
        // recursion(A_L)
        int n1_out;
        RELAPACK(csytrf_rec)(uplo, n_full, &n1, &n1_out, A, ldA, ipiv, Work_L, ldWork, &info1);
        n1 = n1_out;
        ...
        // recursion(A_BR)
        int n2_out;
        RELAPACK(csytrf_rec)(uplo, &n_full2, &n2, &n2_out, A_BR, ldA, ipiv_B, Work_BR, ldWork, &info2);
        ...
        *n_out = n1 + n2;
        ...
        // recursion(A_L)
        int n2_out;
        RELAPACK(csytrf_rec)(uplo, n_full, &n2, &n2_out, A, ldA, ipiv, Work_R, ldWork, &info2);
        n2 = n2_out;
        ...
        // recursion(A_TL)
        int n1_out;
        RELAPACK(csytrf_rec)(uplo, &n_full1, &n1, &n1_out, A, ldA, ipiv, Work_L, ldWork, &info1);
