...
void RELAPACK(ssytrf_rec)(const char *, const int *, const int *, int *, 
    float *, const int *, int *, float *, const int *, int *);


void RELAPACK(ssytrf)(
    ...
    *info = 0;
    ...
    int nout;
    RELAPACK(ssytrf_rec)(&cleanuplo, n, n, &nout, A, ldA, ipiv, cleanWork, ldA, info);

    if (cleanWork != Work)
        free(cleanWork);
}

void RELAPACK(ssytrf_rec)(
    ...
    if (*uplo == 'L') {
        ...
        float *const Work_BR = (*n == *n_full) ? Work : Work + *ldWork * n1 + n1;
        ...
        // recursion(A_BR)
        int n2_out;
        RELAPACK(ssytrf_rec)(uplo, &n_full2, &n2, &n2_out, A_BR, ldA, ipiv_B, Work_BR, &ldWork_BR, &info2);
        ...
        *n_out = n1 + n2;
        ...
        float *const Work_R = (*n == *n_full) ? Work : Work + *ldWork * n1;
        ...
        float *const Work_L  =                   Work;
        float *const Work_TR = (*n == *n_full) ? Work + *ldWork * n2_diff + n_rest : Work + *ldWork * n1 + n_rest;
        ...
        // recursion(A_TL)
        int n1_out;
        RELAPACK(ssytrf_rec)(uplo, &n_full1, &n1, &n1_out, A, ldA, ipiv, Work_L, &ldWork_L, &info1);
