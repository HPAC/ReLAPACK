...
int main(int argc, char* argv[]) {
    ...
    { // L
        ...
        // run
        RELAPACK(zsytrf_rook)("L", &n, A1, &n, ipiv1, Work, &lWork, &info);
        LAPACK(zsytf2_rook)("L", &n, A2, &n, ipiv2, &info);

        // check error
        const double error = z2vecerr(n * n, A1, A2) + i2vecerr(n, ipiv1, ipiv2);
        printf("zsytrf_rook L:\t%g\n", error);
    }
    ...
    { // U
        ...
        // run
        RELAPACK(zsytrf_rook)("U", &n, A1, &n, ipiv1, Work, &lWork, &info);
        LAPACK(zsytf2_rook)("U", &n, A2, &n, ipiv2, &info);

        // check error
        const double error = z2vecerr(n * n, A1, A2) + i2vecerr(n, ipiv1, ipiv2);
        printf("zsytrf_rook U:\t%g\n", error);
    }
