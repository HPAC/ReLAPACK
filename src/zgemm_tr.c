...
void RELAPACK(zgemm_tr)(const char *transA, const char *transB, const char *uplo,
    ...
    const double *const A_B = A + 2 * (tranA ? *ldA * n1 : n1);

    // B_L B_R
    const double *const B_L = B;
    const double *const B_R = B + 2 * (tranB? n1 : *ldB * n1);
    ...
    // recursion(z_TL)
    RELAPACK(zgemm_tr)(transA, transB, uplo, &n1, k, alpha, A_T, ldA, B_L, ldB, beta, C_TL, ldC);
    ...
    // recursion(z_BR)
    RELAPACK(zgemm_tr)(transA, transB, uplo, &n2, k, alpha, A_B, ldA, B_R, ldB, beta, C_BR, ldC);
}
