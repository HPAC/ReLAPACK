...
void RELAPACK_sgbtrf(
    ...
    *info = 0;
    ...
    float *Wl = malloc(mWl * nWl * sizeof(float));
    float *Wu = malloc(mWu * nWu * sizeof(float));
    LAPACK(slaset)("G", &mWl, &nWl, ZERO, ZERO, Wl, &mWl);
    LAPACK(slaset)("G", &mWu, &nWu, ZERO, ZERO, Wu, &mWu);

    // Recursive kernel
    RELAPACK_sgbtrf_rec(m, n, kl, ku, Ab, ldAb, ipiv, Wl, &mWl, Wu, &mWu, info);
...
static void RELAPACK_sgbtrf_rec(
    ...
    RELAPACK_sgbtrf_rec(m, &n1, kl, ku, Ab_L, ldAb, ipiv_T, Wl, ldWl, Wu, ldWu, info);

    // Wl = A_BLb
    LAPACK(slacpy)("U", &m22, &n1, A_BLb, ldA, Wl, ldWl);
    ...
    for (i = 0; i < mn1; i++) {
        ...
        if (ip != i)
            if (ip < *kl)
                ...
                BLAS(sswap)(&i, A_L + i, ldA, Wl + ip - *kl, ldWl);
    }
    ...
    // Wu = A_TRr
    LAPACK(slacpy)("L", &m1, &n22, A_TRr, ldA, Wu, ldWu);
    // Wu = A_TL \ Wu
    BLAS(strsm)("L", "L", "N", "U", &m1, &n22, ONE, A_TL, ldA, Wu, ldWu);
    // A_TRr = Wu
    LAPACK(slacpy)("L", &m1, &n22, Wu, ldWu, A_TRr, ldA);
    // A_BRtl = A_BRtl - A_BLt * A_TRl
    BLAS(sgemm)("N", "N", &m21, &n21, &n1, MONE, A_BLt, ldA, A_TRl, ldA, ONE, A_BRtl, ldA);
    // A_BRbl = A_BRbl - Wl * A_TRl
    BLAS(sgemm)("N", "N", &m22, &n21, &n1, MONE, Wl, ldWl, A_TRl, ldA, ONE, A_BRbl, ldA);
    // A_BRtr = A_BRtr - A_BLt * Wu
    BLAS(sgemm)("N", "N", &m21, &n22, &n1, MONE, A_BLt, ldA, Wu, ldWu, ONE, A_BRtr, ldA);
    // A_BRbr = A_BRbr - Wl * Wu
    BLAS(sgemm)("N", "N", &m22, &n22, &n1, MONE, Wl, ldWl, Wu, ldWu, ONE, A_BRbr, ldA);
    ...
    RELAPACK_sgbtrf_rec(&m2, &n2, kl, ku, Ab_BR, ldAb, ipiv_B, Wl, ldWl, Wu, ldWu, info);
    if (*info)
        *info += n1;
    // shift pivots
    for (i = 0; i < mn2; i++)
