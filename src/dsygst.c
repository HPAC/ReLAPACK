...
void LARPACK(dsygst)(const int *itype, const char *uplo, const int *n,
    ...
    *info = 0;
    ...
    // Constants
    // 0, 1, -1, 1/2, -1/2
   	const double ZERO[] = {0}, ONE[] = {1}, MONE[] = {-1}, HALF[] = {.5}, MHALF[] = {-.5};
    // 1
    const int IONE[] = {1};
...
#ifdef ALLOW_MALLOC
            // T = -1/2 * B_BL * A_TL
            BLAS(dsymm)("R", "L", &n2, &n1, MHALF, A_TL, ldA, B_BL, ldB, ZERO, T, &n2);
            // A_BL = A_BL + T
            for (i = 0; i < n1; i++)
                BLAS(daxpy)(&n2, ONE, T + n2 * i, IONE, A_BL + *ldA * i, IONE);
#else
...
#ifdef ALLOW_MALLOC
            // T = -1/2 * A_TL * B_TR
            BLAS(dsymm)("L", "U", &n1, &n2, MHALF, A_TL, ldA, B_TR, ldB, ZERO, T, &n1);
            // A_TR = A_BL + T
            for (i = 0; i < n2; i++)
                BLAS(daxpy)(&n1, ONE, T + n1 * i, IONE, A_TR + *ldA * i, IONE);
#else
...
#ifdef ALLOW_MALLOC
            // T = 1/2 * A_BR * B_BL
            BLAS(dsymm)("L", "L", &n2, &n1, HALF, A_BR, ldA, B_BL, ldB, ZERO, T, &n2);
            // A_BL = A_BL + T
            for (i = 0; i < n1; i++)
                BLAS(daxpy)(&n2, ONE, T + n2 * i, IONE, A_BL + *ldA * i, IONE);
#else
...
#ifdef ALLOW_MALLOC
            // T = 1/2 * B_TR * A_BR
            BLAS(dsymm)("R", "U", &n1, &n2, HALF, A_BR, ldA, B_TR, ldB, ZERO, T, &n1);
            // A_TR = A_TR + T
            for (i = 0; i < n2; i++)
                BLAS(daxpy)(&n1, ONE, T + n1 * i, IONE, A_TR + *ldA * i, IONE);
#else
