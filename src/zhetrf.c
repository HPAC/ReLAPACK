...
void RELAPACK(zhetrf_rec)(const char *, const int *, const int *, int *, 
...
void RELAPACK(zhetrf_rec)(
    ...
    if (*uplo == 'L') {
        ...
        double *const Work_BR = (*n == *n_full) ? Work : Work + 2 * *ldWork * n1 + 2 * n1;
        ...
        // recursion(A_BR)
        int n2_out;
        RELAPACK(zhetrf_rec)(uplo, &n_full2, &n2, &n2_out, A_BR, ldA, ipiv_B, Work_BR, &ldWork_BR, &info2);
        ...
        *n_out = n1 + n2;
        ...
        double *const Work_R = (*n == *n_full) ? Work : Work + 2 * *ldWork * n1;
        ...
        double *const Work_L  =                   Work;
        double *const Work_TR = (*n == *n_full) ? Work + 2 * *ldWork * n2_diff + 2 * n_rest : Work + 2 * *ldWork * n1 + 2 * n_rest;
        ...
        // recursion(A_TL)
        int n1_out;
        RELAPACK(zhetrf_rec)(uplo, &n_full1, &n1, &n1_out, A, ldA, ipiv, Work_L, &ldWork_L, &info1);
