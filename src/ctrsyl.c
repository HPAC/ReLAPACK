...
void LARPACK(ctrsyl)(const char *tranA, const char *tranB, const int *isgn,
    ...
    *info = 0;
    ...
    // Constants
    // 1, -1, -isgn
   	const float c1[] = {1, 0}, cm1[] = {-1, 0}, cmsgn[] = {-*isgn, 0};
    // 0
    const int i0 = 0;
    ...
    if (*m > *n) {
        ...
        if (notransB) {
            ...
            // C_R = C_R -/+ C_L * B_TR
            BLAS(cgemm)("N", "N", m, &n2, &n1, cmsgn, C_L, ldC, B_TR, ldB, scale1, C_R, ldC);
            // recusion(A, B_BR, C_R)
            ...
            // C_L = C_L -/+ C_R * B_TR'
            BLAS(cgemm)("N", "C", m, &n1, &n2, cmsgn, C_R, ldC, B_TR, ldB, scale1, C_L, ldC);
            // recusion(A, B_TL, C_L)
