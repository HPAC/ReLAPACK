...
void LARPACK(strsyl)(const char *tranA, const char *tranB, const int *isgn,
    ...
    *info = 0;
    ...
    // Constants
    // 1, -1, -isgn
   	const float s1 = 1, sm1 = -1, smsgn = -*isgn;
    // 0
    const int i0 = 0;
    ...
    if (*m > *n) {
        ...
        if (notransB) {
            ...
            // C_R = C_R -/+ C_L * B_TR
            BLAS(sgemm)("N", "N", m, &n2, &n1, &smsgn, C_L, ldC, B_TR, ldB, scale1, C_R, ldC);
            // recusion(A, B_BR, C_R)
            ...
            // C_L = C_L -/+ C_R * B_TR'
            BLAS(sgemm)("N", "C", m, &n1, &n2, &smsgn, C_R, ldC, B_TR, ldB, scale1, C_L, ldC);
            // recusion(A, B_TL, C_L)
