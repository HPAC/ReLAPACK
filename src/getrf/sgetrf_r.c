...
void sgetrf_r(const int *m, const int *n, float *A, const int *ldA, int *ipiv, int *info) {
...
#define ipiv_B (ipiv + n1)
    ...
    // A_TR = A_TL \ A_TR
    LAPACK(strsm)("L", "L", "N", "U", &n1, &n2, s1, A_TL, ldA, A_TR, ldA);
    // A_BR = A_BR - A_BL * A_TR
    LAPACK(sgemm)("N", "N", &rm, &n2, &n1, sm1, A_BL, ldA, A_TR, ldA, s1, A_BR, ldA);
    // A_BR = LU(A_BR)
    sgetrf_r(&rm, &n2, A_BR, ldA, ipiv_B, info);
    if (*info) {
        *info += n1;
        return;
    }
...
#define A_R (A + *ldA * mn)
    ...
    // A_R = A_S \ A_R
    LAPACK(strsm)("L", "L", "N", "U", &mn, &rn, s1, A_S, ldA, A_R, ldA);
}
